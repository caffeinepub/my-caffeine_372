...
export default function FamilyTreePage({
  ...
  function handleTreeExport() {
    ...
    // Build recursive SVG-style HTML tree
    function buildTreeHTML(parentId: string | null, depth: number): string {
      const children = nodes.filter((n) => n.parentId === parentId);
      if (!children.length) return "";
      const genColors = [
        "#1a4d2e",
      ...
      let html = `<div style="display:flex;flex-direction:row;align-items:flex-start;gap:0;justify-content:center;">`;
      for (const node of children) {
        const color = genColors[node.generationLevel % genColors.length];
...
.header {
  ...
  padding-bottom: 8px; border-bottom: 4px solid #1a4d2e; margin-bottom: 10px;
}
.logo-wrap {
  width: 64px; height: 64px; border-radius: 50%; overflow: hidden;
  border: 3px solid #1a4d2e; background: #f0fdf4; flex-shrink: 0;
  display: flex; align-items: center; justify-content: center;
}
.org-info { flex: 1; }
.bismillah { font-size: 15px; color: #1e40af; margin-bottom: 3px; }
.org-name { font-size: 26px; font-weight: 700; letter-spacing: 1px; }
.apon { color: #0f766e; }
.foundation { color: #b91c1c; }
.org-address { font-size: 12px; color: #d97706; margin-top: 3px; }
.doc-title { text-align: right; font-size: 22px; font-weight: 700; color: #1a4d2e; }
.doc-sub { text-align: right; font-size: 11px; color: #6b7280; margin-top: 3px; }
.watermark {
...
.footer {
  ...
  font-size: 10px; color: #9ca3af; position: relative; z-index: 1;
}
.print-btn {
  background: linear-gradient(135deg, #1a4d2e, #14532d);
  color: white; border: none; padding: 12px 36px; border-radius: 10px;
  cursor: pointer; font-size: 16px; font-family: 'Hind Siliguri', sans-serif;
  margin: 12px auto 0; display: block; font-weight: 600;
...
<div class='page'>
  ...
  <div class='header'>
    ...
    <div class='org-info'>
      ...
      <div class='org-name'><span class='apon'>আপন</span> <span class='foundation'>ফাউন্ডেশন</span></div>
      <div class='org-address'>${orgAddress}</div>
    </div>
    <div>
      <div class='doc-title'>বংশপরম্পরা Tree চার্ট</div>
      <div class='doc-sub'>মোট: ${nodes.length} জন | প্রজন্ম: ${maxGen + 1}টি</div>
      <div class='doc-sub'>তারিখ: ${today}</div>
    </div>
  </div>
  <div class='tree-container'>${treeHTML}</div>
  <div class='footer'>
    <span>আপন ফাউন্ডেশন | বংশপরম্পরা Tree চার্ট</span>
    <span>তারিখ: ${today}</span>
  </div>
</div>
<button class='print-btn' onclick="window.print()">📥 8K ল্যান্ডস্কেপ PDF ডাউনলোড করুন</button>
</body></html>`;
          ...
          <>
            ...
            <div className="mt-6 flex justify-center">
              <Button
                ...
                data-ocid="familytree.tree_pdf_button"
              >
                <Download size={16} />
                ৮K/১৬K ল্যান্ডস্কেপ PDF ডাউনলোড করুন
              </Button>
            </div>
          </>
        )}
